/**
 * Sound manager for multi_press_release format
 * Supports both press and release sounds with MP3 files
 */

import { SoundManager, SoundPackConfig, SoundSystemOptions } from '../types/SoundTypes';

export class MultiPressReleaseSoundManager implements SoundManager {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      console.log('MultiPressReleaseSoundManager initialized');
    } catch (error) {
  ...
  async loadSoundPack(packPath: string): Promise<void> {
    try {
      ...
      console.log(`Loaded multi_press_release sound pack: ${config.name}`);
  ...
  private async preloadCommonSounds(): Promise<void> {
    ...
    // Preload sounds for common keys
    const commonKeys = ['30', '31', '32', '57', '28']; // A, S, D, Space, Enter
    const preloadPromises: Promise<any>[] = [];
    
    for (const keyCode of commonKeys) {
      const pressSound = this.getSoundForKey(keyCode, false);
      const releaseSound = this.getSoundForKey(keyCode, true);
      
      if (pressSound) {
        preloadPromises.push(this.loadAudioBuffer(pressSound));
      }
      if (releaseSound) {
        preloadPromises.push(this.loadAudioBuffer(releaseSound));
      }
    }
  ...
  private getSoundForKey(keyCode: string, isRelease: boolean = false): string | null {
    if (!this.currentSoundPack) return null;

    const lookupKey = isRelease ? `${keyCode}-up` : keyCode;
    const specificSound = this.currentSoundPack.defines[lookupKey];
    
    if (specificSound && typeof specificSound === 'string') {
      return specificSound;
    }
    
    // Fallback to generic sounds
    if (isRelease && this.currentSoundPack.sound) {
      // For release, try to use soundup pattern or fallback to main sound
      const soundup = (this.currentSoundPack as any).soundup;
      if (soundup) {
        return soundup.replace('{0-4}', '0'); // Use first generic release sound
      }
    }
    
    if (this.currentSoundPack.sound) {
      // Use random generic sound (R0-R4)
      const randomRow = Math.floor(Math.random() * 5);
      return this.currentSoundPack.sound.replace('{0-4}', randomRow.toString());
    }
  ...
  async playKeySound(keyCode: string, isRelease: boolean = false): Promise<void> {
    ...
    const soundFile = this.getSoundForKey(keyCode, isRelease);
    if (!soundFile) {
      console.warn(`No sound defined for key ${keyCode} ${isRelease ? '(release)' : '(press)'}`);
      return;
    }

...
// import * as colorConvert from "color-convert"; // Not allowed in Shopify SDK
import colorwayTemplate from "../config/colorways/colorway_template.json";
import { subscribe } from "../stateAdapter";
...
export default class ColorUtil {
  ...
  static getAccent(cw_name) {
    cw_name = cw_name || stateAdapter.getState().colorways.active;
  ...
  static getUiAccent(cw_name, defaultAccent) {
    ...
    // let hsv = colorConvert.hex.hsv(accent); // Disabled - not allowed in Shopify SDK
    let hsv = [0, 0, 100]; // Fallback
    let ratio = this.contrast(accent, "202024");
    //too dark
    if (ratio < 7) {
      let ratioDelta = 7 - ratio;
      hsv[2] = Math.min(hsv[2] + 10 * ratioDelta, 100);
      // let hex = `#${colorConvert.hsv.hex(hsv)}`; // Disabled - not allowed in Shopify SDK  
      let hex = accent; // Fallback to original color
      return hex;
    }
    return accent;
  }
  ...
  static addCodeToOverride(key_code, swatch) {
    swatch = swatch || stateAdapter.getState().colorways.activeSwatch;
    let cw = JSON.parse(JSON.stringify(this.getUserColorway()));
    if (!cw || !swatch) return;
    cw.override[key_code] = swatch;
    // TODO: Implement custom colorway updates in React state
  ...
  static contrast(hex1, hex2) {
    // let rgb1 = colorConvert.hex.rgb(hex1); // Disabled - not allowed in Shopify SDK
    // let rgb2 = colorConvert.hex.rgb(hex2); // Disabled - not allowed in Shopify SDK
    let rgb1 = [255, 255, 255]; // Fallback
    let rgb2 = [0, 0, 0]; // Fallback
  ...
  static getUiAccentText(cw_name) {
    ...
    // let v = colorConvert.hex.hsv(acc); // Disabled - not allowed in Shopify SDK
    let v = [0, 0, 100]; // Fallback
  ...
  static getTransparentColor(hex, transparency) {
    // let val = colorConvert.hex.rgb(hex); // Disabled - not allowed in Shopify SDK
    let val = [255, 255, 255]; // Fallback
